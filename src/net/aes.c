#include "aes.h"
#include <string.h>
#include <stdio.h>

// --- SHA1 Implementation for Key Derivation ---
#define ROL(x, n) (((x) << (n)) | ((x) >> (32 - (n))))

static void sha1_transform(uint32_t state[5], const uint8_t buffer[64]) {
    uint32_t w[80], a, b, c, d, e, i;
    for (i = 0; i < 16; i++) {
        w[i] = (buffer[i*4] << 24) | (buffer[i*4+1] << 16) | (buffer[i*4+2] << 8) | buffer[i*4+3];
    }
    for (i = 16; i < 80; i++) {
        w[i] = ROL(w[i-3] ^ w[i-8] ^ w[i-14] ^ w[i-16], 1);
    }
    a = state[0]; b = state[1]; c = state[2]; d = state[3]; e = state[4];
    for (i = 0; i < 80; i++) {
        uint32_t f, k;
        if (i < 20) { f = (b & c) | ((~b) & d); k = 0x5A827999; }
        else if (i < 40) { f = b ^ c ^ d; k = 0x6ED9EBA1; }
        else if (i < 60) { f = (b & c) | (b & d) | (c & d); k = 0x8F1BBCDC; }
        else { f = b ^ c ^ d; k = 0xCA62C1D6; }
        uint32_t temp = ROL(a, 5) + f + e + k + w[i];
        e = d; d = c; c = ROL(b, 30); b = a; a = temp;
    }
    state[0] += a; state[1] += b; state[2] += c; state[3] += d; state[4] += e;
}

static void sha1(const uint8_t *data, size_t len, uint8_t hash[20]) {
    uint32_t state[5] = {0x67452301, 0xEFCDAB89, 0x98BADCFE, 0x10325476, 0xC3D2E1F0};
    uint8_t buffer[64];
    uint64_t bitlen = (uint64_t)len * 8;
    size_t i;

    for (i = 0; i < len / 64; i++) {
        sha1_transform(state, data + i * 64);
    }
    size_t remaining = len % 64;
    memcpy(buffer, data + i * 64, remaining);
    buffer[remaining++] = 0x80;
    if (remaining > 56) {
        memset(buffer + remaining, 0, 64 - remaining);
        sha1_transform(state, buffer);
        remaining = 0;
    }
    memset(buffer + remaining, 0, 56 - remaining);
    for (i = 0; i < 8; i++) buffer[56 + i] = (bitlen >> ((7 - i) * 8)) & 0xFF;
    sha1_transform(state, buffer);

    for (i = 0; i < 5; i++) {
        hash[i*4] = (state[i] >> 24) & 0xFF;
        hash[i*4+1] = (state[i] >> 16) & 0xFF;
        hash[i*4+2] = (state[i] >> 8) & 0xFF;
        hash[i*4+3] = state[i] & 0xFF;
    }
}

void AES_DeriveKey(const char *password, uint8_t key[16]) {
    uint8_t hash[20];
    sha1((const uint8_t*)password, strlen(password), hash);
    memcpy(key, hash, 16); // Use first 16 bytes as AES-128 key
}

// Tiny AES implementation based on public domain code
// Optimized for simplicity and size.

static const uint8_t sbox[256] = {
    };

static uint32_t sub_word(uint32_t w) {
    return (sbox[(w >> 24) & 0xFF] << 24) | (sbox[(w >> 16) & 0xFF] << 16) |
           (sbox[(w >> 8) & 0xFF] << 8) | sbox[w & 0xFF];
}

static uint32_t rot_word(uint32_t w) {
    return (w << 8) | (w >> 24);
}

void AES_Init(AES_Ctx *ctx, const uint8_t key[16]) {
    uint32_t rcon[] = {0x01000000, 0x02000000, 0x04000000, 0x08000000, 0x10000000,
                       0x20000000, 0x40000000, 0x80000000, 0x1B000000, 0x36000000};
    int i;
    for (i = 0; i < 4; i++) {
        ctx->round_keys[i] = (key[i * 4] << 24) | (key[i * 4 + 1] << 16) |
                             (key[i * 4 + 2] << 8) | key[i * 4 + 3];
    }
    for (i = 4; i < 44; i++) {
        uint32_t temp = ctx->round_keys[i - 1];
        if (i % 4 == 0) {
            temp = sub_word(rot_word(temp)) ^ rcon[i / 4 - 1];
        }
        ctx->round_keys[i] = ctx->round_keys[i - 4] ^ temp;
    }
}

// I'll rewrite the AES block encryption to be more robust.
// Re-implementing based on specialized tiny AES code.

#define RJ_MUL(x) (((x) << 1) ^ ((((x) >> 7) & 1) * 0x1b))

static void MixColumns(uint8_t *s) {
    for (int i = 0; i < 4; i++) {
        uint8_t a = s[i*4];
        uint8_t b = s[i*4+1];
        uint8_t c = s[i*4+2];
        uint8_t d = s[i*4+3];
        uint8_t h = a ^ b ^ c ^ d;
        s[i*4]   ^= h ^ RJ_MUL(a ^ b);
        s[i*4+1] ^= h ^ RJ_MUL(b ^ c);
        s[i*4+2] ^= h ^ RJ_MUL(c ^ d);
        s[i*4+3] ^= h ^ RJ_MUL(d ^ a);
    }
}

static void SubBytesShiftRows(uint8_t *s) {
    uint8_t t;
    for(int i=0; i<16; i++) s[i] = sbox[s[i]];
    t = s[1]; s[1] = s[5]; s[5] = s[9]; s[9] = s[13]; s[13] = t;
    t = s[2]; s[2] = s[10]; s[10] = t; t = s[6]; s[6] = s[14]; s[14] = t;
    t = s[15]; s[15] = s[11]; s[11] = s[7]; s[7] = s[3]; s[3] = t;
}

static void AddRoundKey(uint8_t *s, const uint32_t *rk) {
    for (int i = 0; i < 4; i++) {
        uint32_t w = rk[i];
        s[i*4]   ^= (w >> 24) & 0xFF;
        s[i*4+1] ^= (w >> 16) & 0xFF;
        s[i*4+2] ^= (w >> 8)  & 0xFF;
        s[i*4+3] ^= w & 0xFF;
    }
}

static void aes_encrypt_block_v2(AES_Ctx *ctx, uint8_t *s) {
    AddRoundKey(s, &ctx->round_keys[0]);
    for (int r = 1; r < 10; r++) {
        SubBytesShiftRows(s);
        MixColumns(s);
        AddRoundKey(s, &ctx->round_keys[r * 4]);
    }
    SubBytesShiftRows(s);
    AddRoundKey(s, &ctx->round_keys[40]);
}

void AES_CTR_Xcrypt(AES_Ctx *ctx, const uint8_t iv[16], uint8_t *data, size_t size) {
    uint8_t counter[16];
    uint8_t stream[16];
    memcpy(counter, iv, 16);
    
    for (size_t i = 0; i < size; i++) {
        if (i % 16 == 0) {
            memcpy(stream, counter, 16);
            aes_encrypt_block_v2(ctx, stream);
            // Increment counter (big endian)
            for (int j = 15; j >= 0; j--) {
                if (++counter[j] != 0) break;
            }
        }
        data[i] ^= stream[i % 16];
    }
}
